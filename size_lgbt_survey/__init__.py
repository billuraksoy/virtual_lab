...
class Player(BasePlayer):
    ...
    marital = make_list_field("What is your marital status? (choose one)",
    ...
    children = models.IntegerField(
        label="How many children less than 18 years of age live in your household? If none, please put 0. Number of children:",
        min=0, max=100)
    education = make_list_field(
        "What is the highest level of education you've completed? (choose one) (If currently enrolled, mark the previous grade or highest degree received.)",
        ["High school, GED, or less", "Some college credits, no degree", "Associate's degree (for example: AA, AS)",
         "Bachelor’s degree or equivalent (for example: BA, BS)",
         "Master’s degree or higher (for example: MA, MS, MEng, MEd, MSW, MBA, MD, DDS, DVM, LLB, JD, PhD, EdD)"])
    employment = make_list_field("Are you currently…?",
    ...
    rural = make_list_field("What type of community do you live in?",
                            ["Rural area", "Large city", "Small city or town", "Suburb near a large city"])
    ...
    trans_emp_disc = make_yn_field(
        "Do you think the law should prohibit employment discrimination against transgender individuals?")

    # page 3
    trans_house_disc = make_yn_field(
        "Do you think the law should prohibit housing discrimination against transgender individuals?")
    ...
    gay_emp_disc = make_yn_field(
        "Do you think the law should prohibit employment discrimination against lesbian, gay, or bisexual individuals?")
    gay_house_disc = make_yn_field(
        "Do you think the law should prohibit housing discrimination against lesbian, gay, or bisexual individuals?")
    gay_neighbor = make_yn_field(
        "Would you be comfortable having an openly lesbian, gay, or bisexual person as a neighbor?")

    # insert
    refuse = make_yn_field(
        "Do you think that private businesses (such as cake decorators or florists) should be able to refuse service to same-sex couples or other LGBTQ+ individuals for religious reasons?")

    # page 7
    # NOTE: Sex at birth is not a binary, should update that.
    your_sex = models.StringField(
        label="What sex were you assigned at birth, on your original birth certificate? (choose one)",
    ...
    sexuality = make_list_field("Which of the following best represents how you think of yourself?",
                                ["Gay or Lesbian", "Straight, that is, not gay or lesbian", "Bisexual",
                                 "Something else", "I don’t know the answer"])
    a_check_2 = make_list_field(
        "Before providing an answer, one should always read the text carefully. To check whether you have been reading the text carefully, we ask you to select the third option below as your answer.",
    ...
    attraction = make_list_field(
        "People are different in their sexual attraction to other people. Which category below best describes your feelings?",
        ["Only attracted to females", "Mostly attracted to females", "Equally attracted to females and males",
         "Mostly attracted to males", "Only attracted to males", "Other (please specify below)"])
    attr_other = models.StringField(label="", blank=True)

    # Page 8
    religion = make_list_field("What is your current religious affiliation?",
                               ["Christian (any denomination)", "Jewish", "Muslim (any denomination)", "Hindu",
                                "Buddhist ", "Asian Folk Religion (e.g., Taoist, Confucian) ", "I am not religious ",
                                "Some other religious affiliation (please specify below)"])
    religion_oth = models.StringField(label="", blank=True)
    religion_raised = make_list_field(
        "Which of the following religious affiliations best describes how you were raised?",
        ["Christian (any denomination)", "Jewish", "Muslim (any denomination)", "Hindu", "Buddhist ",
         "Asian Folk Religion (e.g., Taoist, Confucian) ", "I was not raised in any religion ",
         "Some other religious affiliation (please specify below)"])
    religion_raised_oth = models.StringField(label="", blank=True)
    religion_imp = make_list_field("How important is religion in your life?",
                                   ["Very important", "Somewhat important", "Not too important",
                                    "Not at all important"])

    party = make_list_field(
        "Generally speaking, do you usually think of yourself as a Republican, Democrat, or Independent/Other? Choose the option that best describes you.",
    ...
    income = make_list_field("What is your household income before taxes?",
                             ["Less than $20,000", "$20,000 - $39,999", "$40,000 - $59,999", "$60,000 - $79,999",
                              "$80,000 - $99,999", "$100,000 - $149,999", "$150,000 - $199,999", "$200,000 or higher"])
    ...
    pop_manager = models.IntegerField(
        label="Out of every 100 people in the general US population, I think approximately _____ out of 100 would be comfortable having a transgender manager at work.",
        min=0, max=100)
    pop_emp = models.IntegerField(
        label="Out of every 100 people in the general US population, I think approximately _____ out of 100 would agree that the law should prohibit employment discrimination against transgender individuals.",
        min=0, max=100)
...
class SurveyPage7(Page):
    ...
    form_model = 'player'
    form_fields = ["your_sex", "Ma", "Fe", "Tr", "Nb", "straight", "sexuality", "a_check_2", "same_sex", "attraction",
                   "attr_other"]
...
class SurveyPage8(SurveyPage):
    template_name = "size_lgbt_survey/Survey_Basic_NoI.html"
    form_fields = ["religion", "religion_oth", "religion_raised", "religion_raised_oth", "religion_imp", "party",
                   "pol_spectrum", "a_check_3", "pres_2016", "pres_2020", "income"]
...
page_sequence = [survey_intro, SurveyPage1, SurveyPage1b, SurveyPage2, SurveyPage3, SurveyPage4, SurveyPage5,
                 SurveyPage6, SurveyPage7, SurveyPage8, survey_1, SurveyPage13, SurveyPageEnd]
